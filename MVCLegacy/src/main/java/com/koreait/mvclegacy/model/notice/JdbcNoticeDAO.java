...
/*
 	...
 	[ Mybatis ] 를 
 	SQL Mapper : 쿼리문과 자바객체간 매핑(연결)..
 	
 	[JDBC 자체]
 	
 	[ Hibernate ]
 	ORM(Object Relation Mapping) : 자바객체와 관계형 데이터베이스 와의 매핑 	
 	[JPA]
 	.... 	
*/
...
public class JdbcNoticeDAO implements NoticeDAO{
	...
	public List selectAll() {
		...
		list = jdbcTemplate.query(sql, new Object[] {}, new RowMapper<Notice>() {// 인터페이스라서 내부익명클래스로 처리를 해줘야 한다..재정의..rs 메서드를 Notice에 채워넣자.			
			public Notice mapRow(ResultSet rs, int rowNum) throws SQLException {
				logger.info("mapRow 메서드 호출, rowNum = "+rowNum);
	...
	public Notice select(int notice_id) {
		...
		// 내부익명클래스 리턴을 아래 코드가 받아준다..안되면 nullpoint에러 발생!!!
