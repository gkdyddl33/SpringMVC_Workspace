...
public class PaymentController {
	...
	public String delCart(HttpSession session) throws CartException{
		// 장바구니를 삭제하기 위해서는 로그인한 회원만 가능..
		if(session.getAttribute("member")==null) {
			throw new LoginRequiredException("로그인이 필요한 서비스입니다.");
		}
		paymentService.delete((Member)session.getAttribute("member"));
		return "redirect:/shop/cart/list";	// 장바구니 비어있는 모습..
	}
	
	// 장바구니 주문 수량 수정하기
	@RequestMapping(value="/shop/cart/edit",method = RequestMethod.POST)
	public ModelAndView  editCart(@RequestParam("cart_id") int[] cartArray, @RequestParam("quantity") int[] qArray) {//@RequestParam("컴포넌트이름=name..")
		...
		// 수정 되었다는 메시지를 보고 싶다면? message.jsp로 응답하자.
		MessageData messageData = new MessageData();
		messageData.setResultCode(1);
